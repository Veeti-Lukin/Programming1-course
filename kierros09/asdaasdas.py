...
def add_friendship(network, name1, name2):
    ...
    for name in [name1, name2]:
        if not name.isalpha() or name1 == name2:  # sanity check
            print("Error: '{name}' is not a valid name: friendship not added.")
            return

    if name1 not in network:  # Adding relationships
        network[name1] = []  # between name1 and name2
    if name2 not in network[name1]:  # both added to both's
        network[name1].append(name2)  # network
...
def print_command(network, namelist):
    ...
    for name1 in sorted(network):  # formatted correctly and
        print(name1)  # in alphabetical order
        for name2 in sorted(network[name1]):
            print(f"- {name2}")
...
def friends_command(network, namelist):
    ...
    if name not in network:  # unknown person check
        print(f"Error: '{name}' is an unknown name.")
        return

    for friend in sorted(network[name]):  # printed friendslist in
        print(friend)  # in alphabetical order
...
def common_friends_command(network, namelist):
    ...
    for name in namelist:
        ...
        if name not in namelist:  # unknown person check2
            print(f"Error: '{name}' is an unknown name.")
            return
    ...
    name1 = namelist[0]
    name2 = namelist[1]
    if name1 == name2:  # no common friends with themselves
        print(f"{name1} has no common friends with him/herself.")

    # finding the common friends of name1 and name2

    for friend in network[name1]:
        if friend in sorted(network[name2]) and friend in sorted(
                network[name2]):
            commonfriendlist.append(friend)

    if len(commonfriendlist) == 0:  # error msg if no common friends
        print(f"{name1} and {name2} have no common friends.")
        return

    for friend in sorted(commonfriendlist):
        print(friend)
