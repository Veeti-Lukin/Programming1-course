...
class Fraction:
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, Fraction):
            ...
            expanded_other_numerator = int(
                other_in_parts[0]) * self.__denominator
...
def main():
    ...
    while mainloop:
        ...
        if command == ADD_COMMAND:
            fraction_string = input(
                "Enter a fraction in the form integer/integer: ")
            fraction_name = input("Enter a name: ")
            try:
                numerator, denominator = int(fraction_string.split("/")[0]), \
                                         int(fraction_string.split("/")[1])
                memory[fraction_name] = Fraction(numerator, denominator)
            ...
            if fraction_name in memory:
                print(f"{fraction_name} = {memory.get(fraction_name)}")
            else:
            ...
            fraction_name = input("1st operand: ")
            if fraction_name not in memory:
                print(f"Name {fraction_name} was not found")
                continue

            fraction_name2 = input("2nd operand: ")
            if fraction_name2 not in memory:
                print(f"Name {fraction_name2} was not found")

            else:
                new = memory[fraction_name].multiply(memory[fraction_name2])
                print(
                    f"{memory[fraction_name]} * {memory[fraction_name2]} = {new}")
                new.simplify()
            ...
            try:
                with open(filename, mode="r") as f:
                    for line in f:
                        ...
                        name, numerator, denominator = parts[0], int(parts[1]), \
                                                       int(parts[2])
                        memory[name] = Fraction(numerator, denominator)
