...
class MovingBox:
    ...
    def add_item(self, item_name, amount):
        ...
        :param amount: int, how many times item is to be added
        :return: None
        :raise: ValueError if amount is negative number
        """
        if amount < 0:
            raise ValueError
    ...
    def remove_item(self, item_name, amount):
        """
        Removes *amount* of one *item* from box contents.
        Checks if the item is at least once in the box and that there is enough to remove *amount* from.
        Assumes right types of parameters but tests if *amount* is positive.
        :param item_name: str, name of the item
        :param amount: int, how many times item is to be removed
    ...
    def list_content(self):
        for key, val in self.__contents.items():
            print(key, val)
